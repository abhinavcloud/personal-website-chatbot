...
def _extract_title_from_text(text: str, fallback: str) -> str:
    ...
    the content is empty or nothing usable is found.
    """
...
def _fetch_title_for_doc(doc_id: str, first_chunk_key: str, fallback: str) -> str:
    """
    Read just the first ~1KB of a document's first chunk (a cheap Range GET,
    not the whole chunk) and derive its real title from the content. This
    happens once per document inside list_documents_by_type, so the model
    always sees the actual title up front and never has a cheaper-but-wrong
    filename-slug option to fall back on.
    """
    try:
        ...
        return _extract_title_from_text(text, fallback)
    except Exception as e:
        logger.warning("Could not fetch title for doc_id=%s: %s", doc_id, e)
        return fallback
...
def list_documents_by_type(source_type: str) -> str:
    """
    List every document available in one category, with its doc_id and its
    actual title (read from the document's own content, not guessed from the
    filename), plus chunk count. Use this FIRST whenever the user asks you to
    ...
    single search result. The titles here are real, so you can name and
    describe documents directly from this list; use list_chunks_for_document
    and get_document_chunks afterward only when you need a document's full
    content, not just its identity.

    Args:
        source_type: One of "blog", "project", or "resume".

    Returns:
        A list of documents in that category (doc_id, real title, total chunk
        count), or an error message if source_type is invalid or nothing was
        found.
    """
    ...
    for doc_id, count in sorted(doc_chunk_counts.items()):
        ...
        title = _fetch_title_for_doc(doc_id, first_key, fallback) if first_key else fallback
        lines.append(f'- doc_id="{doc_id}" | title="{title}" | total_chunks={count}')
