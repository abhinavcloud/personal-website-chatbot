...
def list_documents_by_type(source_type: str) -> str:
    """
    List every document available in one category, with a short doc_id and
    approximate title for each. Use this FIRST whenever the user asks you to
    ...
    single search result. After listing, use list_chunks_for_document and
    get_document_chunks on the specific ones relevant to the question.

    Args:
        source_type: One of "blog", "project", or "resume".

    Returns:
        A list of documents in that category (doc_id, approximate title, total
        chunk count), or an error message if source_type is invalid or nothing
        was found.
    """
    ...
    for page in paginator.paginate(Bucket=STAGING_BUCKET, Prefix=f"{STAGING_PREFIX}/"):
        for obj in page.get("Contents", []):
            ...
            if name.endswith(".metadata.json"):
                continue
            base, _ = _parse_chunk_filename(name)
    ...
    for doc_id, count in sorted(doc_chunk_counts.items()):
        ...
        lines.append(f'- doc_id="{doc_id}" | approx_title="{readable}" | total_chunks={count}')
