...
def retrieve_tool(params: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Bedrock's Retrieve API caps numberOfResults at 100 PER CALL -- there is
    no way to request more in a single request. For anything that needs
    the full corpus for a source_type (recency sorting, exhaustive resume
    listing), fetch_all=True pages through nextToken until Bedrock reports
    no more results, instead of silently working from whatever fit in one
    100-result page (which is query-text-dependent and was previously
    producing a different, wrong "most recent" answer depending on how the
    ...
    fetch_all = params.get("fetchAll", False)
    max_pages = int(params.get("maxPages", 20))  # safety cap: 20 pages * 100 = up to 2000 chunks
    ...
    def build_payload(n: int) -> List[Dict[str, Any]]:
        ...
        return [
            {"knowledgeBaseId": kb_id, "retrievalQuery": {"text": text_query},
    ...
    payloads = build_payload(per_page)

    # Find a payload variant that works, and remember which one for pagination.
    ...
    if fetch_all:
        ...
        while next_token and pages_fetched < max_pages:
            ...
            try:
                ...
                logger.warning("Pagination request failed on page %d, stopping with partial results: %s", pages_fetched + 1, e)
                break
...
def retrieve_then_stitch_and_ask(agent: Agent, user_question: str) -> str:
    ...
    # questions need the FULL population for the source type, not just
    # whatever fits in one 100-result page (Bedrock's Retrieve API caps
    # numberOfResults at 100 per call, so "just ask for more" doesn't work
    # -- fetchAll pages through nextToken to actually cover the corpus).
    need_full_corpus = source_type == "resume" or recency_intent

    params = {
        "text": user_question,
        "knowledgeBaseId": STRANDS_KNOWLEDGE_BASE_ID,
        "numberOfResults": 100 if need_full_corpus else 50,
        "region": REGION,
        "sourceType": source_type,
        "fetchAll": need_full_corpus,
    }
    logger.info("Calling retrieve_tool with source_type=%s recency_intent=%s fetch_all=%s",
                source_type, recency_intent, need_full_corpus)
